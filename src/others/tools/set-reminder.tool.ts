...
export function setReminderTool(content: string, timestamp: string): void {
  console.log(
    `----\nSetting the following reminder for ${timestamp}:\n${content}\n----`,
  );
}

export const setReminderSchema: AssistantTool = {
  name: 'set_reminder',
  description:
  ...
  input_schema: {
    ...
    properties: {
      ...
      timestamp: {
        type: 'string',
        description:
          'The exact date and time when the reminder should be triggered, formatted as an ISO 8601 timestamp (YYYY-MM-DDTHH:MM:SS) or a Unix timestamp. The system handles all timezone processing internally, ensuring reminders are triggered at the correct time regardless of where the user is located.',
      },
    },
